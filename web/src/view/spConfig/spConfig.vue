<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="市场IDs" prop="marketplaceIds" width="120" />
        <el-table-column align="left" label="SP-API客户端id" prop="clientId" width="120" />
        <el-table-column align="left" label="SP-API客户端密钥" prop="clientSecret" width="120" />
        <el-table-column align="left" label="刷新token" prop="refreshToken" width="120" />
        <el-table-column align="left" label="AWS IAM User Access Key Id" prop="accessKeyId" width="120" />
        <el-table-column align="left" label="AWS IAM User Secret Key" prop="secretKey" width="120" />
        <el-table-column align="left" label="AWS Region" prop="region" width="120" />
        <el-table-column align="left" label="AWS IAM Role ARN" prop="roleArn" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateSpConfigFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getSpConfigList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteSpConfigByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSpConfigFunc = async(row) => {
    const res = await findSpConfig({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSpConfigFunc = async (row) => {
    const res = await deleteSpConfig({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
